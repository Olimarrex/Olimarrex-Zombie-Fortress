...
const SColor color_toxicshroom(255, 255, 60, 140);
//fauna
...
class PNGLoader
{
	...
	if (pixel == color_tile_ground) {
		...
		else if (pixel == color_toxicshroom)
		{
			spawnBlob( map, "tshroom", offset, -1);
			offsets[autotile_offset].push_back( offset );
		}
		//ANIMALS
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		...
		{
			if (moddedIsTileSolid(map, map.getTile(offset + map.tilemapwidth))) // load trees only at the ground
			{
...
void PlaceMostLikelyTile( CMap@ map, int offset )
{
	...
	else if ( moddedIsTileSolid(map, down) &&
