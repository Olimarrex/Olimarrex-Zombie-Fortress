...
void onTick(CBlob@ this)
{
	...
	{
		knight.state = KnightStates::normal; //cancel any attacks or grappuling
		knight.swordTimer = 0;
		knight.doubleslash = false;
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			{
				DoAttack(this, 1.5f, -(vec.Angle()), 120.0f, Hitters::sword, delta, knight);
			}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	{
		HandleGrappleCommand(this, params, !canSend(this));
	}
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
				{
					...
					bool bone = isTileBone(hi.tile);
					if (ground || wood || dirt_stone || gold || bone)
					{
...
//rcon CBlob@ b = server_CreateBlob('boneguard', 1, getPlayerByUsername('ollimarrex').getBlob().getPosition()); b.server_SetPlayer(getPlayerByUsername('ollimarrex'));