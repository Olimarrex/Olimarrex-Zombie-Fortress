...
void onTick(CBlob@ this)
{
	...
	const bool inair = (!this.isOnGround() && !this.isOnLadder() && !this.isInWater());
	u32 timeInAir;
	if(inair)
	{
		timeInAir = this.get_u32("timeInAir");
	...
	this.set_u32("timeInAir", timeInAir);
	         ...
	         (pressed_a2 || (specialShieldState)))
	{
		...
		else
		{
			...
			if (knight.state == KnightStates::shieldgliding && !this.isInWater() && !forcedrop)
			{
				...
				//Don't apply gliding - we have our wings instead.
			...
			else if (knight.state == KnightStates::shielddropping)
			{
				if (this.isInWater())
				{
					//if (vel.y > 0.5f && Maths::Abs(vel.x) * 3 > Maths::Abs(vel.y))
					{
	...
	{
		Vec2f vel = this.getVelocity();
		f32 flyingSpeed = Maths::Max(timeInAir / 160.0f, 1.025f);
		flyingSpeed = flying_force_y / float(flyingSpeed);
		if(vel.y > 0)  //counter downward force without need of time in air
		{
			flyingSpeed -= (vel.y * vel.y ) / 8.0f;
		}
	    this.AddForce(Vec2f(this.isKeyPressed(key_left) ? -flying_force_x : this.isKeyPressed(key_right) ? flying_force_x : 0.0f, flyingSpeed));
	}
}
