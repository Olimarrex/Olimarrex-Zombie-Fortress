...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	// tilemap check
	const bool buildSolid = (moddedIsTileSolid(map, buildTile) || (blob !is null && blob.isCollidable()));
	...
	else if (moddedIsTileSolid(map, backtile) || map.hasTileSolidBlobs(backtile))
	{
		if (!buildSolid && !map.hasTileSolidBlobsNoPlatform(backtile) && !moddedIsTileSolid(map, backtile))
		{
