...
void Vehicle_StandardControls( CBlob@ this, VehicleInfo@ v )
{
        ...
        for (uint i = 0; i < aps.length; i++)
        {
            ...
            if (blob !is null && ap.socket)
            {
                ...
                if (ap.name == "FLYER")
                {
					...
					else if (this.getName() == "glider")
                    {
                        v.fly_amount = 0.8f;
                    }
					else if (this.getName() == "balloon")
                    {
                        v.fly_amount = 0.9f;
                    }
    ...
    if(this.hasTag("airship"))
    {
		...
		CInventory@ inv = this.getInventory();
		const int amount = 1;
		if(inv !is null && inv.isInInventory("mat_wood", 1))
		{
...
bool takeFuel(CInventory@ this, int amount)
{
	if(this.isInInventory("mat_wood", amount))
	{
		this.server_RemoveItems("mat_wood", amount);
		return true;
	}
	return false;
}