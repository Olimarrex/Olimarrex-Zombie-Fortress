...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
	...
	if (cmd == this.getCommandID("summon zombies"))
	{
		...
		CBlob@ zombie;
		CBlob@[] zombiesCount;
		getBlobsByName("skeleton1", @zombiesCount);
		
		for (int i = 0; i < zombiesToSpawnCount; i++)
		{
			if (zombiesCount.length >= maxZombies)
			{
				@zombie = server_CreateBlob( "skeleton", caller.getTeamNum(), this.getPosition()); 
			...
			{
				@zombie = server_CreateBlob( "skeleton", caller.getTeamNum(), caller.getAimPos()); 
			}
