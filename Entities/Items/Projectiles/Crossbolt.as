...
{
	if (blob !is null && doesDamageBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		else
		{
			...
			if (dmg > 0.0f)
			{
				...
				this.setVelocity(this.getVelocity() / 1.2f);
			}
		}

		if (dmg > 0.0f && this.getShape().vellen < 3)   // dont stick bomb arrows
		{
			this.Tag("collided");
		}
	}
...
{
	if(this.getShape().vellen < 3)
	{
		return(doesDamageBlob(this, blob));
	}
	return false;
}
