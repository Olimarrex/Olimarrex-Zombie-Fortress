...
void onInit(CBlob@ this)
{
	...
	if(getRules().hasTag("alwaysNight"))
	{
		ShopItem@ s = addShopItem(this, "Sun Lantern", "$sunlantern$", "sunlantern", "Emits light that crops can grow by", false);
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("shop made item"))
	{
		...
		string name = params.read_string();
		{
			...
			{
				CBlob@ c = server_CreateBlob("tree_pine", 0, this.getPosition());		
			}
			
			if(name == "summontreebushy")
			{
				CBlob@ d = server_CreateBlob("tree_bushy", 0, this.getPosition());
			}	
		}
	}
}
